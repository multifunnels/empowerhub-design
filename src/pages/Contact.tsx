            ...
            <div>
              <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
                <div className="flex justify-center space-x-4 mb-2">
                  <div className={`p-3 rounded-full cursor-pointer transition-colors flex items-center gap-2 ${contactMethod === "whatsapp" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`} onClick={() => setContactMethod("whatsapp")}>
                    <MessageSquare className="h-5 w-5" />
                    <span>שליחה בווטסאפ</span>
                  </div>
                  <div className={`p-3 rounded-full cursor-pointer transition-colors flex items-center gap-2 ${contactMethod === "email" ? "bg-primary/10 text-primary" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`} onClick={() => setContactMethod("email")}>
                ...
                <div>
                  ...
                  <Input id="name" value={name} onChange={e => setName(e.target.value)} required className="text-right" dir="rtl" />
                </div>
                <div>
                  <Label htmlFor="email">אימייל</Label>
                  <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} required className="text-right" dir="rtl" />
                </div>
                <div>
                  <Label htmlFor="message">הודעה</Label>
                  <Textarea id="message" value={message} onChange={e => setMessage(e.target.value)} required className="min-h-[150px] text-right" dir="rtl" />
                </div>
                <Button type="submit" className={`w-full ${contactMethod === "whatsapp" ? "bg-green-600 hover:bg-green-700" : ""}`} disabled={isSubmitting}>
                  {isSubmitting ? "שולח..." : contactMethod === "whatsapp" ? "פתיחת ווטסאפ" : "שלח הודעה"} 
                  {contactMethod === "whatsapp" ? <MessageSquare className="mr-2 h-4 w-4" /> : <Send className="mr-2 h-4 w-4" />}
                </Button>
                <p className="text-xs text-gray-500 text-center">
                  {contactMethod === "whatsapp" ? "*לחיצה על הכפתור תפתח את יישומון הווטסאפ" : "*הטופס יפתח את תוכנת הדואר האלקטרוני שלך"}
                </p>
              </form>
            </div>
          </div>
